...
if ! codex plugin list --json | node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>process.exit(JSON.parse(s).installed.some(x=>x.pluginId==='coart@coart-public'&&x.version==='0.2.4')?0:1))"; then
